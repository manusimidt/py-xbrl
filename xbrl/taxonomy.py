...
def load_mappings() -> dict[str, str]:
    """
    Load all JSON taxonomy mappings from a directory into a single dict.
    """
    dir_path = Path(__file__).parent / "taxonomies"
    _ns_schema_map = {}
    for file in dir_path.glob("*.json"):
        with open(file, "r", encoding="utf-8") as f:
            data = json.load(f)
            _ns_schema_map.update(data)

    return _ns_schema_map


ns_schema_map: dict[str, str] = load_mappings()
